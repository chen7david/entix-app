...
- For URL-backed filters:
  ...
  - Seed search from the URL **once on mount**. Never recreate `initialFilters` with a new object every render (for example `{ search: urlValue }`) when passing it into `DataTableWithFilters` — that used to reset the input mid-typing.
