import { AI_REQUEST_TIMEOUT_MS, GEMINI_MODELS } from "@api/constants/ai.constants";
    ...
    private async runChat(
        ...
        try {
            ...
            if (msg.includes("abort")) {
                throw new ServiceUnavailableError(
                    `Gemini request timed out (${AI_REQUEST_TIMEOUT_MS / 1000}s) for model "${this.model}".`
                );
            }
...
/** @deprecated Use {@link GeminiAiService}. Kept for existing imports/tests. */
export const AiService = GeminiAiService;

/** Default Gemini model when env omits GEMINI_MODEL. */
export const GEMINI_DEFAULT_MODEL = GEMINI_MODELS.DEFAULT;
