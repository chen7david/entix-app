...
export async function createUserAndOrganization(
    ...
    await expect(page.getByText("Creation Successful")).toBeVisible();
}
...
export async function activateCurrencyIfNeeded(page: Page, currencyCode: "CNY" | "ETD") {
    const card = page.locator(".ant-card").filter({ hasText: currencyCode }).first();
    await expect(card).toBeVisible();

    const activateButton = card.getByRole("button", { name: "Activate Wallet" });
...
async function ensureFundingAccountIfMissing(drawer: Locator) {
...
export async function creditOrganizationFromSuperAdmin(
    ...
    // Organization select
    const orgFormItem = drawer
        .locator(".ant-form-item")
        .filter({ hasText: "Organization Context" });
    await orgFormItem.locator(".ant-select").click();
    await page.getByTitle(input.organizationName).click();

    await ensureFundingAccountIfMissing(drawer);

    // Currency select
    const currencyFormItem = drawer.locator(".ant-form-item").filter({ hasText: "Currency" });
    await currencyFormItem.locator(".ant-select").click();
    await page.getByTitle(input.currencyCode).click();

    // Amount input
    const amountFormItem = drawer.locator(".ant-form-item").filter({ hasText: "Amount" });
    await amountFormItem.locator("input").fill(input.amount);
