...
export const financialSessionPaymentEvents = sqliteTable(
    "financial_session_payment_events",
    {
        ...
        eventType: text("event_type", {
            enum: ["paid", "refunded", "manual_paid", "manual_reset"],
        }).notNull(),

        // Link to the financial_transactions record.
        ...
        // Stored here so history is self-contained even if the transaction is reversed.
        amountCents: integer("amount_cents"),

        // Who triggered this event — system user ID for automated payments,
        // admin user ID for manual overrides.
        performedBy: text("performed_by")
            .notNull()
            .references(() => authUsers.id, { onDelete: "restrict" }),

        // Optional human-readable reason, required for manual overrides.
        note: text("note"),

        // Immutable — no updatedAt. Use createdAt as the event timestamp.
    ...
    (t) => [
        ...
        // Prevents duplicate manual events per type per student per session (when transactionId is NULL)
