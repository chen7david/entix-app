    ...
    return (
        ...
        >
            ...
            >
                <Text strong>
                    {hasActiveUploads
                        ? `Uploading ${queue.filter((t) => t.status === "uploading" || t.status === "pending" || t.status === "processing").length} files...`
                        : "Uploads Complete"}
                </Text>
                <Space size="small">
                    <Button
            ...
            {!isMinimized && (
                ...
                >
                    {queue.map((task) => (
                        <div
                            key={task.id}
                            className="mb-2 p-2 rounded hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors"
                        >
                            <div className="flex justify-between items-center mb-1">
                                <div className="flex items-center gap-2 overflow-hidden pr-2">
                                    {task.type === "video" ? (
                                        <VideoCameraOutlined className="text-gray-500 flex-shrink-0" />
                                    ) : task.type === "audio" ? (
                                        <AudioOutlined className="text-gray-500 flex-shrink-0" />
                                    ) : (
                                        <FileOutlined className="text-gray-500 flex-shrink-0" />
                                    )}
                                    <Text
                                        ellipsis
