...
-- Backfill organization_id for org-owned accounts (where legacy null exists) to satisfy NOT NULL constraint
UPDATE `financial_accounts` SET `organization_id` = `owner_id` WHERE `owner_type` = 'org' AND `organization_id` IS NULL;
...
CREATE TABLE `__new_financial_accounts` (
	...
	`is_funding_account` integer DEFAULT false NOT NULL,
...
--> statement-breakpoint
INSERT INTO `__new_financial_accounts`("id", "owner_id", "owner_type", "currency_id", "organization_id", "name", "balance_cents", "is_active", "archived_at", "created_at", "updated_at", "is_funding_account", "account_type") SELECT "id", "owner_id", "owner_type", "currency_id", "organization_id", "name", "balance_cents", "is_active", "archived_at", "created_at", "updated_at", "is_funding_account", "account_type" FROM `financial_accounts`;--> statement-breakpoint
